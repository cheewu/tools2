...
public class Correlation {
	...
	public static void buildIndex() {
		...
		System.out
				.println("Mongo articicle finished,Begin to sort keywordCount map....");

		keywordCountMap = MapUtil.sortByValue(unsortedKeywordCountMap);

		System.out
				.println("Map sort finished,begin to write file keyworcount ");

		try {
			FileWriter fw = new FileWriter("KeywordCount.txt");
			for (Map.Entry<Integer, Integer> entry : keywordCountMap.entrySet()) {
				fw.write(getStringbyID(entry.getKey()) + "   "
						+ entry.getValue() + "\n");
				fw.flush();
			}
			fw.close();
			System.out
					.println("keyword count finished,begin to write keyword invert index。。。");
	...
	public static void analyzerRelation(Set<Integer> kwCategories,
			Set<Integer> candidateCategories, int minOccur, int minRatio,
		...
		for (Map.Entry<Integer, Integer> m : keywordCountMap.entrySet()) {
			...
			Integer kw = m.getKey();
			Integer kwcategory = KeywrodQuery
					.getKeywordCategory(getStringbyID(kw));
			...
			if (kwCategories.contains(kwcategory)) {

				System.out.println("Begin to get relation for: "
						+ getStringbyID(m.getKey()) + "  " + m.getValue());
				...
				for (int paragraph : kwParagraphList) {
					List<Integer> paragraphWords = paragraphKeyword
							.get(paragraph);
					for (Integer kwcandidate : paragraphWords) {

						Integer canCategory = KeywrodQuery
								.getKeywordCategory(getStringbyID(kwcandidate));

						if (!candidateCategories.contains(canCategory)
								|| kwcandidate.equals(kw))
							continue;
				...
				// 计算candidatemap中的关键词在包含kw的关键词的文档中出现的比例
				// 这个比例要比关键词在整个文档中出现的比例高MINRATIO倍;
				Map<Integer, Double> candidateScoreMap = new HashMap<Integer, Double>();

				for (Map.Entry<Integer, Integer> entry : candidateMap
						.entrySet()) {
					...
					double baseratio = (double) keywordCountMap
							.get(kwcandidate) / (500);
					double joinratio = ((double) 10000 * kwcandidateCount / (double) keywordCountMap
							.get(kw));
					...
					if (joinratio > minRatio * baseratio) {
						double score = Math.sqrt(kwcandidateCount)
								* Math.log(joinratio / baseratio);
						fw.write(getStringbyID(kw)
								+ ", "
								+ score
								+","
								+ getStringbyID(kwcandidate)
								+ " ,"
								+ kwcandidateCount
								+ " ,"
								+ ddf1.format(baseratio)
								+ " ,"
								+ ddf1.format(joinratio)
								+ " ,"
								+ KeywrodQuery
										.getKeywordCategory(getStringbyID(kwcandidate))
								+ "\n");
				...
				Map<Integer, Double> sortedCandidateMap = MapUtil
						.sortByValue(candidateScoreMap);

				// 把一个关键词所有的关系按照类别归类
				// country,province,destination,poi,food,item,note
				// 每个类别对应一个hashmap
				Map<String, Map<String, Double>> categoryCorrelationMap = new HashMap<String, Map<String, Double>>();

				for (Map.Entry<Integer, Double> entry : sortedCandidateMap
						.entrySet()) {

					String category = KeywrodQuery
							.getKeywordCategoryName(getStringbyID(entry
									.getKey()));
					if (categoryCorrelationMap.containsKey(category)) {
						categoryCorrelationMap.get(category)
								.put(getStringbyID(entry.getKey()),
										entry.getValue());
					} else {
						Map<String, Double> oneCategoryMap = new HashMap<String, Double>();
						oneCategoryMap.put(getStringbyID(entry.getKey()),
								entry.getValue());
						categoryCorrelationMap.put(category,
								MapUtil.sortByValue(oneCategoryMap));
	...
	public static void main(String[] args) {
		...
		if (args.length < 5) {
			System.out
					.println("Usage Correlation <ListKeywordCategories>  <ListCandidateCategories> <MinOccur>  <MinRatio> <isInsert>");
			System.exit(0);
		}

		String[] catKw = args[0].split(",");
		String[] catCan =args[1].split(",");
		Integer occur = Integer.parseInt(args[2]);
		Integer ratio = Integer.parseInt(args[3]);
		boolean insert = false;
		if (args[2].equals("true")) {
			insert = true;
		}

		Set<Integer> kwList = new HashSet<Integer>();
		Set<Integer> canList = new HashSet<Integer>();

		for (int j = 0; j < catKw.length; j++) {

			kwList.add(Integer.parseInt(catKw[j]));
		}

		for (int k = 0; k < catCan.length; k++) {

			canList.add(Integer.parseInt(catCan[k]));
		}
		...
		buildIndex();
		try {
			analyzerRelation(kwList, canList, occur, ratio, insert);
		} catch (IOException e) {
